...
const Ticker = ({onAddTicker,allCoinsList})=>{
...
if (ticker.length > 0){
  ...
  filteredCoins = allCoinsList.filter((coin)=>{
    return coin.toLowerCase().includes(ticker.toLowerCase())
  })
}
...
const handleChange = useCallback((v)=>setTicker(v),[])
       ...
       <>
          ...
          {autoCompleteItems.map((autoCompleteItem, index) =>
              <span
                ...
                {autoCompleteItems.map((autoCompleteItem,id) => {
                  return (
                    <span
